...
# ============================================================
# 🎌 HIT ANIME GIF — pre-cached on startup to avoid rate limits
# ============================================================
...
_gif_cache: list = []          # in-memory URL cache
_gif_local_dir = "/tmp/hilex_gifs"   # local mp4 store
_gif_prefetch_done = False

async def _prefetch_gifs():
    """Called once at startup: fetch 5 batches × 50 gifs, download & store locally."""
    global _gif_cache, _gif_prefetch_done
    import os as _os
    _os.makedirs(_gif_local_dir, exist_ok=True)
    # If we already have local files use them
    existing = [f for f in _os.listdir(_gif_local_dir) if f.endswith(".mp4")]
    if existing:
        _gif_cache = [f"{_gif_local_dir}/{f}" for f in existing]
        random.shuffle(_gif_cache)
        _gif_prefetch_done = True
        print(f"[gif] Loaded {len(_gif_cache)} cached gifs from disk")
        return
    print("[gif] Pre-fetching anime gifs (5 batches × 50)...")
    urls_collected = []
                ...
                async with s.get(url, timeout=aiohttp.ClientTimeout(total=12)) as r:
                    if r.status == 200:
                        ...
                        for item in data.get("data", []):
                            ...
                            if mp4:
                                urls_collected.append(mp4)
            except Exception as e:
                print(f"[gif] batch {batch} error: {e}")
            await asyncio.sleep(0.5)   # gentle pacing between requests
    print(f"[gif] Downloading {len(urls_collected)} gifs locally...")
    downloaded = 0
    async with aiohttp.ClientSession() as s:
        for idx, mp4_url in enumerate(urls_collected[:200]):
            local_path = f"{_gif_local_dir}/gif_{idx:04d}.mp4"
            if _os.path.exists(local_path):
                _gif_cache.append(local_path)
                downloaded += 1
                continue
            try:
                async with s.get(mp4_url, timeout=aiohttp.ClientTimeout(total=15)) as r:
                    if r.status == 200:
                        data = await r.read()
    ...
    random.shuffle(_gif_cache)
    _gif_prefetch_done = True
    print(f"[gif] ✅ Pre-fetch done — {downloaded} gifs stored locally")

async def fetch_random_anime_gif() -> str | None:
    """Return a local gif path. Falls back to None on error."""
    global _gif_cache
    try:
        if _gif_cache:
            path = _gif_cache.pop(random.randint(0, len(_gif_cache)-1))
            import os as _os
            if _os.path.exists(path):
                return path
        # cache empty — try a quick live fetch as fallback
        tag = random.choice(GIPHY_TAGS)
        url = (
            f"https://api.giphy.com/v1/gifs/search"
            f"?api_key={GIPHY_API_KEY}&q={tag.replace(' ', '+')}"
...
# ============================================================
# 🎨 STYLED BUTTONS — Bot API 9.4 (Feb 2026)
#   • style:               "default" | "primary" | "secondary"
#                          | "success" | "warning" | "danger"
#   • icon_custom_emoji_id: premium custom emoji shown on button
# Telethon doesn't wrap these yet, so we pass them via raw TL.
# ============================================================
from telethon.tl.types import KeyboardButtonCallback

def _raw_btn(text: str, data: bytes, style: str = "default", emoji_id: str | None = None):
    """Build a raw KeyboardButtonCallback with optional style/emoji (Bot API 9.4)."""
    btn = KeyboardButtonCallback(text=text, data=data)
    # Patch in the new fields supported by Bot API 9.4+
    # These are ignored by older clients / TL versions that don't know them,
    # so the button still works everywhere.
    if style and style != "default":
        btn.style = style          # "primary" | "secondary" | "success" | "warning" | "danger"
    if emoji_id:
        btn.icon_custom_emoji_id = emoji_id
    return btn

# Premium custom-emoji IDs for buttons (same set as message emojis)
# These are the animated sticker doc IDs from your PREMIUM_EMOJI_IDS dict.
# Bots can use these in buttons when the bot owner has Telegram Premium.
_BTN_EMOJI = {
    "shopify":  "6298612102709909362",  # ✅
    "razorpay": "5420323339723881652",  # ⚠️
    "proxy":    "5440681540541502133",  # ❌  (placeholder — swap for a 📡 sticker ID)
    "cc":       "6298612102709909362",
    "plan":     "5440681540541502133",
    "stats":    "6298612102709909362",
    "feedback": "5420323339723881652",
    "help":     "5440681540541502133",
    "admin":    "6298612102709909362",
    "live":     "6298612102709909362",
    "charged":  "6298612102709909362",
    "dead":     "5440681540541502133",
    "stop":     "5440681540541502133",
    "back":     "5420323339723881652",
    "copy":     "6298612102709909362",
}

# ============================================================
# 🔡 SMALL-CAPS FONT CONVERTER  (ʜᴇʟᴇx style)
# ============================================================
_SC = str.maketrans(
    "abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ",
    "ᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴩqʀꜱᴛᴜᴠᴡxʏᴢ ᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴩqʀꜱᴛᴜᴠᴡxʏᴢ"
)
def to_sc(text: str) -> str:
    """Convert plain ASCII letters to Unicode small-caps (ʜᴇʟᴇx font style)."""
...
<b>🤖 Bot By ➜ <a href="tg://user?id={ADMIN_ID}">{OWNER_NAME}</a></b>"""
    ...
    buttons = [
        [
            _raw_btn(f"🔥 ʟɪᴠᴇ ({approved})", f"live_{message_id}".encode(), style="success"),
            _raw_btn(f"💎 ᴄʜᴀʀɢᴇᴅ ({charged})", f"charged_{message_id}".encode(), style="primary")
        ],
        [
            _raw_btn(f"💀 ᴅᴇᴀᴅ ({dead})", f"dead_{message_id}".encode(), style="danger"),
            _raw_btn("🛑 sᴛᴏᴩ", f"stop_{message_id}".encode(), style="danger")
        ]
    ]
    try:
...
╚══〔 ʜ ᴇ ʟ ᴇ x  メ 〕══╝"""
        buttons = [
            [_raw_btn("📋 ᴄᴏᴩʏ ᴄᴄ", f"copycc_{result['card']}".encode(), style="secondary")]
        ]
...
async def send_hit_to_gc(result, user_id, first_name):
    ...
    try:
        ...
        gc_msg = (
            f"<b>{status_icon} ʜᴇʟᴇx ʜɪᴛ · {status_text}</b>\n"
            f"<blockquote>{response_msg}</blockquote>\n\n"
            f"💳 <b>ᴄᴀʀᴅ</b>  <tg-spoiler><code>{masked}</code></tg-spoiler>\n"
            f"💰 <b>ᴀᴍᴏᴜɴᴛ</b>  <code>{currency}{price}</code>\n"
            f"🔗 <b>ɢᴀᴛᴇ</b>  <code>{gateway}</code>\n\n"
            f"🏦 <b>ʙɪɴ</b>  <code>{result['card'][:6]}</code> · {brand}\n"
            f"🏛 <b>ʙᴀɴᴋ</b>  {bank}\n"
            f"🌍 <b>ᴄᴏᴜɴᴛʀʏ</b>  {country} {flag}\n\n"
            f"⏱ <code>{current_time}</code>  ·  "
            f"<a href=\"tg://user?id={user_id}\">{first_name}</a> <i>[{plan}]</i>\n"
            f"<i>ᴘᴏᴡᴇʀᴇᴅ ʙʏ ʜᴇʟᴇx</i>"
        )
...
╚══〔 ʜ ᴇ ʟ ᴇ x  メ 〕══╝"""
    ...
    buttons = [
        [
            _raw_btn("⚡ sʜᴏᴘɪꜰʏ", b"shopify_tools", style="primary"),
            _raw_btn("💎 ʀᴀᴢᴏʀᴩᴀʏ", b"rz_tools", style="primary")
        ],
        [
            _raw_btn("📡 ᴩʀᴏxʏ", b"proxy_tools", style="secondary"),
            _raw_btn("🃏 ᴄᴄ ᴛᴏᴏʟs", b"cc_tools", style="secondary")
        ],
        [
            _raw_btn("👑 ᴩʟᴀɴ", b"premium_tools", style="primary"),
            _raw_btn("📊 sᴛᴀᴛs", b"bot_stats", style="secondary")
        ],
        [
            _raw_btn("💬 ꜰᴇᴇᴅʙᴀᴄᴋ", b"feedback_menu", style="secondary"),
            _raw_btn("🆘 ʜᴇʟᴩ", b"help_menu", style="warning")
        ]
    ]

    if is_admin(user_id):
        buttons.append([_raw_btn("🛠 ᴀᴅᴍɪɴ ᴩᴀɴᴇʟ", b"admin_panel", style="danger")])
...
<b>Choose action:</b>"""
    buttons = [
        [
            _raw_btn("🗝 ɢᴇɴ ᴋᴇʏs", b"admin_genkeys", style="primary"),
            _raw_btn("📈 ꜰᴜʟʟ sᴛᴀᴛs", b"admin_fullstats", style="secondary")
        ],
        [
            _raw_btn("🚫 ʙʟᴏᴄᴋ ᴜsᴇʀ", b"admin_block", style="danger"),
            _raw_btn("✅ ᴜɴʙʟᴏᴄᴋ", b"admin_unblock", style="success")
        ],
        [
            _raw_btn("📋 ʙʟᴏᴄᴋʟɪsᴛ", b"admin_blocklist", style="secondary"),
            _raw_btn("👥 ᴜsᴇʀs ʟɪsᴛ", b"admin_users", style="secondary")
        ],
        [
            _raw_btn("📢 ʙʀᴏᴀᴅᴄᴀsᴛ", b"admin_broadcast", style="primary"),
            _raw_btn("💬 ꜰᴇᴇᴅʙᴀᴄᴋ", b"admin_feedback", style="secondary")
        ],
        [
            _raw_btn("🔙 ʙᴀᴄᴋ", b"back_to_start", style="warning")
        ]
    ]
...
Keys start with <code>HILEX-</code>"""
    await event.edit(premium_emoji(text), buttons=[[_raw_btn("🔙 ᴀᴅᴍɪɴ", b"admin_panel", style="warning")]], parse_mode="html")
...
async def admin_users(event):
    ...
    # reuse /users logic
    await event.edit(premium_emoji("<b>👥 Use /users command for full list + TXT</b>"), buttons=[[_raw_btn("🔙 ᴀᴅᴍɪɴ", b"admin_panel", style="warning")]], parse_mode="html")
...
👑 Goes to {OWNER_NAME}"""
    buttons = [
        [_raw_btn("✍️ ᴡʀɪᴛᴇ ꜰᴇᴇᴅʙᴀᴄᴋ", b"write_feedback", style="primary")],
        [_raw_btn("🔙 ʙᴀᴄᴋ", b"back_to_start", style="warning")]
    ]
    await event.edit(premium_emoji(text), buttons=buttons, parse_mode="html")
...
👑 <a href="tg://user?id={ADMIN_ID}">{OWNER_NAME}</a>"""
    await event.edit(premium_emoji(text), buttons=[[_raw_btn("🔙 ʙᴀᴄᴋ", b"back_to_start", style="warning")]], parse_mode="html")
...
<b>👥 Group:</b> <a href="{GROUP_LINK}">HitByDark</a>"""
    ...
    buttons = [
        [_raw_btn("⚡ sʜᴏᴘɪꜰʏ", b"shopify_tools", style="primary"), _raw_btn("💎 ʀᴀᴢᴏʀᴩᴀʏ", b"rz_tools", style="primary")],
        [_raw_btn("📡 ᴩʀᴏxʏ", b"proxy_tools", style="secondary"), _raw_btn("🃏 ᴄᴄ ᴛᴏᴏʟs", b"cc_tools", style="secondary")],
        [_raw_btn("👑 ᴩʟᴀɴ", b"premium_tools", style="primary"), _raw_btn("📊 sᴛᴀᴛs", b"bot_stats", style="secondary")],
        [_raw_btn("💬 ꜰᴇᴇᴅʙᴀᴄᴋ", b"feedback_menu", style="secondary"), _raw_btn("🆘 ʜᴇʟᴩ", b"help_menu", style="warning")]
    ]
    if is_admin(user_id):
        buttons.append([_raw_btn("🛠 ᴀᴅᴍɪɴ ᴩᴀɴᴇʟ", b"admin_panel", style="danger")])
    try:
...
👑 <a href="tg://user?id={ADMIN_ID}">@{OWNER_USERNAME}</a>"""
    buttons = [
        ...
        [_raw_btn("📊 ᴍʏ ᴩʟᴀɴ", b"my_plan", style="secondary")],
        [_raw_btn("🔙 ʙᴀᴄᴋ", b"back_to_start", style="warning")]
    ]
    await event.edit(premium_emoji(text), buttons=buttons, parse_mode="html")
...
🔑 /redeem KEY
👑 <a href="tg://user?id={ADMIN_ID}">@{OWNER_USERNAME}</a>"""
    await event.edit(premium_emoji(text), buttons=[[_raw_btn("🔙 ʙᴀᴄᴋ", b"premium_tools", style="warning")]], parse_mode="html")
...
👑 <a href="tg://user?id={ADMIN_ID}">@{OWNER_USERNAME}</a>"""), parse_mode="html")
    ...
    await status_msg.edit(
        ...
        buttons=[
            [_raw_btn("🟢 ᴍʏ sɪᴛᴇs", f"chk_my_{status_msg.id}".encode(), style="success"),
             _raw_btn("🔵 ʙᴏᴛ sɪᴛᴇs", f"chk_global_{status_msg.id}".encode(), style="primary")],
            [_raw_btn("❌ ᴄᴀɴᴄᴇʟ", f"cancel_chk_{status_msg.id}".encode(), style="danger")]
        ],
        parse_mode="html"
    )
...
if __name__ == "__main__":
    ...
    while retry_count < 9999:
        try:
            bot.start()
            bot.loop.run_until_complete(_prefetch_gifs())
